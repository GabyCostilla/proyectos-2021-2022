// Reconstrucción del programa Barquitos.
// (C) Versión 3.0 Junio de 2.000, Salvador Pozo
...
class TPanel : public TWindow {
  ...
  private:
	 TWindow		 *Padre; // Ventana padre, sistema
	 HFONT 		 hFont1;	// Fuente usada para coordenadas
    HFONT		 hFont2; // y mensajes

	 TBitmap		 *Bmp[7]; // Mapas de bits del tablero.
    				 // Tableros, barcos del jugador y Odrdenador, auxiliares
    TipoTablero *Jugador, *Ordenador, *Trabajo, *Trabajo2, *Mapa;
	 int			 Raton;	 // Estado del ratón, modo de trabajo
	 int			 Barco[2][4]; // Cuenta de barcos no hundidos del Ordenador y Jugador
    								  // de 4, de 3, de 2 y de 1.
	 bool			 BarSel; // Barco seleccionado en edición
	 int			 Turno;  // Turno de Jugador u Ordenador
    // Variables de estrategia del ordenador:
	 int			 Tocados; // Tocados en el barco actual
	 int			 Estrategia; // Tipo de estrategia a usar
	 int			 Resto; // Casillas con contenido desconocido
	 int			 EstadoOrdenador; // Estado en cadena de decisión de estrategia
	 int 			 t;	  // Variable de depuración.
	 TipoCoor	 Coor[4]; // Posibilidades de tiro

	 TButton 	 *btOk;	// Botón de Ok
	 TRect		 RectBarcos[10]; // Rectángulo conteniendo los diez barcos del jugador en pantalla
	 int			 Lon[10]; // Longitud de cada barco del jugador
    bool			 Ori[10]; // Orientación de los barcos del jugador.
    bool 		 BOri;    // Orientación del barco editado
    int 			 BSel, Xx, Yy, Xp, Yp, Op; // Variables para edición de mapa jugador
	 TRect		 Re; // Rectángulo de edición de mapa de jugador
	 int			 Xd, Yd;	// Coordenadas de disparo
    int			 EstadoJugador;
